...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (loggedInUserStr) {
      try {
        ...
        // Always launch straight to STUDENT_DASHBOARD (or ADMIN_DASHBOARD)
        const initialView = (user.role === 'ADMIN' || user.role === 'SUB_ADMIN') ? 'ADMIN_DASHBOARD' : 'STUDENT_DASHBOARD';
  ...
  const handleLogin = async (user: User) => {
    ...
    // Default ensure profileCompleted is marked true
    const activeUser = { ...user, profileCompleted: true };
    ...
    // Direct Dashboard Redirection (No Onboarding)
    ...
    setState(prev => ({
      ...
      selectedBoard: activeUser.board || 'CBSE',
      selectedClass: activeUser.classLevel || '10',
