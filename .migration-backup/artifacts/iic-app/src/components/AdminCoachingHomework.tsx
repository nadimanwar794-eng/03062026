...
interface CoachingMcq  { id: string; question: string; options: string[]; correctAnswer: number; explanation?: string; }
...
function McqEditor({ mcqs, onChange, accent }:{ mcqs: CoachingMcq[]; onChange:(m:CoachingMcq[])=>void; accent:string }) {
  const add = () => onChange([...mcqs, { id: uid(), question: '', options: ['','','',''], correctAnswer: 0, explanation: '' }]);
        ...
        <div key={m.id} className="border rounded-xl p-3 space-y-2" style={{borderColor:`${accent}30`}}>
          ...
          <div className="grid grid-cols-2 gap-1.5">
            {m.options.map((opt,oi)=>(
              <div key={oi} className={`flex items-center gap-1 border rounded-lg px-2 py-1 ${m.correctAnswer===oi ? 'border-emerald-400 bg-emerald-50':'border-slate-200'}`}>
                <button onClick={()=>upd(m.id,'correctAnswer',oi)}
                  className={`w-3.5 h-3.5 rounded-full border-2 shrink-0 ${m.correctAnswer===oi?'border-emerald-500 bg-emerald-500':'border-slate-300'}`} />
